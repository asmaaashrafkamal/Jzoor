...
try {
  const response = await fetch('http://localhost:8000/delivery/chat/send', {
    method: 'POST',
  ...
  const handleSendMessage = async () => {
    ...
    const getAllMessages =async()=>{
      axios.get(`http://localhost:8000/delivery/driver/messages`,{withCredentials:true}, {
        headers: {
          Authorization: `Bearer ${driverToken}`,
        }
      })
    ...
    const handleContactClick = async (contactId) => {
      ...
      try {
        const res = await axios.get(
          `http://localhost:8000/delivery/chat/admin/${contactId}`,
          {
            params: { delivery_id: userId },
        ...
        if (msgs.length === 0) {
          ...
          const sendRes = await axios.post(
            `http://localhost:8000/delivery/chat/send`,
            {
              sender_id: userId,
              receiver_id: contactId,
              sender_type: 'driver',  // or your appropriate type
  ...
  {messages.length === 0 ? (
    ...
    [...messages]
      .sort((a, b) => new Date(b.time) - new Date(a.time)) // latest at top
      .map((message) => {
        const isDriver = message.senderRole === 'driver';
        const isMe = message.senderId === userId;
        const showOnLeft = !isDriver;
        ...
        return (
          ...
          >
            ...
            >
              ...
              <p className="text-[10px] text-right mt-0.5" style={{ color: showOnLeft ? '#E2E8F0' : '#A0AEC0' }}>
                {message.time}
              </p>
            </div>
            {!showOnLeft && (
