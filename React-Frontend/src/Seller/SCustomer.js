...
useEffect(() => {
  if (!user?.admin_id) return; // wait until user is available

  const id = user.admin_id;
console.log(id);
  axios
    .get(`http://localhost:8000/api/Sget_customer/${id}`)
    .then(res => {
...
console.log(updatedCustomer);
  axios
    .put(`http://localhost:8000/api/Scustomers/${id}`, {
...
const handleConfirmDelete = () => {
  ...
  axios
    .delete(`http://localhost:8000/api/Scustomers/${id}`)
    .then(() => {
