...
const ProductSection = () => {
  ...
  return (
      ...
      <section className="gift relative pt-[60px] pb-[60px] overflow-hidden container" id="Gifts">
        {showToast && (
        ...
        <Title
          ...
          description={`Top picks from ${category.cat_name}`}
        />
                ...
                <div className="p-4">
                  ...
                  <div className="absolute bottom-0 left-0 right-0 translate-y-full group-hover:translate-y-0 transition-all duration-300 p-4 flex flex-col items-center gap-2 bg-white/90">
                    <button
                      ...
                      className="bg-[#af926a] text-white w-full py-2 rounded-full hover:bg-[#8B6F47] transition"
                    >
                      Add To Cart
                    </button>
                    <button
                      onClick={() => handleProductClick(product)}
                      className="bg-[#333]/10 text-[#8B6F47] w-full py-2 rounded-full border-[#8B6F47] hover:bg-[#8B6F47] hover:text-white transition"
                    >
                      More Details
                    </button>
                  </div>
        ...
        {/* Optional: See More Products button */}
        {/* <div className="flex justify-center mt-8">
          <Link
            to={`/ProductList/${category.id}`}
            className="bg-[#4B5929] hover:bg-[#A8C686] text-white font-medium text-lg px-6 py-3 rounded-[10px] transition-all duration-300 transform hover:scale-105 shadow-md"
          >
            See More Products
          </Link>
        </div> */}
